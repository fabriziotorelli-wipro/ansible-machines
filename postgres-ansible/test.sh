...
docker run -d  -p 5432:5432 --privileged -e "MAIN_REPO_URL=$MAIN_REPO_URL" -e "PLAYBOOKS=$PLAYBOOKS" -e "POSTGRES_USER=$POSTGRES_USER" \
          ...
          -v /sys/fs/cgroup:/sys/fs/cgroup -it --name postgres-ansible --user postgres:postgres builditftorelli/postgres-ansible:9.6.2
          # -v /sys/fs/cgroup:/sys/fs/cgroup \
          # -it --name postgres-ansible --volumes-from postgres-ansible-volumes  builditftorelli/postgres-ansible:9.6.2
